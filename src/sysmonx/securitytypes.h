...
{
	constexpr wchar_t* SYSMON_PROVIDER_NAME = L"Microsoft-Windows-Sysmon";
	constexpr wchar_t* POWERSHELL_PROVIDER_NAME = L"Microsoft-Windows-PowerShell";
}

namespace SysmonXDefs
{
	constexpr UINT32 CURRENT_EVENT_VERSION = 1;
	constexpr UINT32 INVALID_PROCESS_ID = 0;
	constexpr UINT32 INVALID_THREAD_ID = 0;
	constexpr UINT32 MIN_PROCESS_ID = 4; //System Process
	constexpr UINT32 DEFAULT_OPCODE = 0;
	constexpr LARGE_INTEGER INVALID_TIMESTAMP = { 0 };
	constexpr UINT32 INVALID_EVENT_ID = 65535;
	...
	//Scanner Aggregation
	static const std::wstring EVENT_SCANNER_AGGREATION_MEM_HOLLOW_MODULES = L"hollow_modules";
	static const std::wstring EVENT_SCANNER_AGGREATION_PARENT_SPOOFING = L"parent_pid_spoofing";
	static const std::wstring EVENT_SCANNER_AGGREATION_CMD_SPOOFING = L"cmdline_spoofing";
...
namespace SysmonXTypes
{
	...
	{
		//Common
		EventUINT32		EventVersion = SysmonXDefs::CURRENT_EVENT_VERSION;
		EventUINT32		EventID = EventID::SECURITY_EVENT_ID_NOT_ASSIGNED;
		EventUINT32		EventCollectorTechID = EventCollectorTechID::EVENT_COLLECTOR_TECH_NA;
		EventUINT32		EventCollectorVectorID = EventCollectorVectorID::EVENT_SUBTYPE_NA;
		EventLARGEINT	EventCreationTimestamp = SysmonXDefs::INVALID_TIMESTAMP;
		EventUINT32		EventCommonProcessID = SysmonXDefs::INVALID_PROCESS_ID;
		EventGUID		EventETWProviderID;
		...
		//EventID
		EventData(const EventUINT32& eventID) : EventID(eventID) {};
	...
	enum EventPropertyID
	{
		...
		EVENT_PROPERTY_COMMON_EventETWProviderID,
	...
	struct ReportData
	{
		ReportChannelID ChannelID = ReportChannelID::REPORT_CHANNEL_DRIVEN_BY_CONFIG;
		std::wstring Name;
		EventObject Event;

		ReportData(EventObject& eventData) : Event(std::move(eventData)) {}
	...
	enum ProcessScanID
	{
		PROCESS_SCAN_ALL = 0x00,
		PROCESS_SCAN_CMDLINE_SPOOFING,
		PROCESS_SCAN_PARENT_PID_SPOOFING,
		PROCESS_SCAN_SUSPICIOUS_MODULES,
		PROCESS_SCAN_SUSPICIOUS_CALL_STACK,
		PROCESS_SCAN_SUSPICIOUS_MEM_REGIONS,
		PROCESS_SCAN_SUSPICIOUS_THREADS,
		PROCESS_SCAN_NA
	};

	enum ProcessScanResult
	{
		PROCESS_SCAN_RESULT_NOT_FOUND = 0x00,
		PROCESS_SCAN_RESULT_FOUND,
		PROCESS_SCAN_RESULT_NA
	};


	struct ScanResultData
	{
		ProcessScanResult Result = ProcessScanResult::PROCESS_SCAN_RESULT_NOT_FOUND;
		WStringCollection FreeText;
	};

	typedef std::vector<ScanResultData> EventDataCollection;

	struct ProcessScanData
	{
		bool IsPerformed = false;
		EventDataCollection Result;
	};

	//Type Helpers
	typedef std::unordered_map<ProcessScanID, ProcessScanData> ProcessScanRequests;

	//============ Cache Related Data ============//
	enum CacheUpdateRequestType
	{
		CACHE_UPDATE_REQUEST_PROCESS = 0x00,
		CACHE_UPDATE_REQUEST_THREAD,
		CACHE_UPDATE_REQUEST_EXCLUSION,
		CACHE_UPDATE_REQUEST_NA
	};

	//Process Cache Container
	struct BaseCacheProcessData
	{
		bool IsRunning = false;
		PROCESS_ID_T ProcessId = SysmonXDefs::INVALID_PROCESS_ID;
		PROCESS_ID_T ParentProcessId = SysmonXDefs::INVALID_PROCESS_ID;
		UINT32 SessionId = 0;
		UINT32 Flags = 0;
		UINT32 TimeDataStamp = 0;
		UINT32 UniqueProcessKey = 0;
		UINT32 UserSID = 0;
		std::wstring ImageName;
		std::wstring CommandLine;
		std::wstring PackageFullName;
		std::wstring PackageRelativeAppId;
		FILETIME Creatime = { 0 };
		std::vector<UINT32> ThreadIDs;
		ProcessScanRequests ScanRequests;
	};

	//Thread Cache Container
	struct BaseCacheThreadData
	{
		bool IsRunning = false;
		PROCESS_ID_T ProcessId = SysmonXDefs::INVALID_PROCESS_ID;
		UINT32 ThreadId = SysmonXDefs::INVALID_PROCESS_ID;
		ULONG64 StackBase = 0;
		ULONG64 StackLimit = 0;
		ULONG64 UserStackLimit = 0;
		ULONG64 UserStackBase = 0;
		ULONG64 StartAddr = 0;
		ULONG64 Win32StartAddr = 0;
		ULONG64 TEBBase = 0;
		ULONG64 CycleTime = 0;
		UINT32 Tokens = 0;
		std::vector<std::wstring> ThreadStartCallStack;
		std::vector<std::wstring> ThreadCurrentCallStack;
	};


	//Exclusions Cache Container
	struct BaseCacheExclusionsData
	{
	...
	//Type Helpers
	typedef std::shared_ptr<BaseCacheProcessData> CacheProcessData;
	typedef std::shared_ptr<BaseCacheThreadData> CacheThreadData;
	typedef std::shared_ptr<BaseCacheExclusionsData> CacheExclusionsData;
	typedef concurrency::concurrent_unordered_map<SysmonXTypes::PROCESS_ID_T, SysmonXTypes::CacheProcessData> ProcessCacheContainer;
	typedef concurrency::concurrent_unordered_map<SysmonXTypes::THREAD_ID_TYPE, SysmonXTypes::CacheThreadData> ThreadCacheContainer;
	typedef concurrency::concurrent_unordered_map<SysmonXTypes::PROCESS_ID_T, SysmonXTypes::CacheExclusionsData> ExclusionsCacheContainer;

	//Cache Data Container
	struct CacheData
	{
		CacheUpdateRequestType Type = CacheUpdateRequestType::CACHE_UPDATE_REQUEST_NA;
		CacheProcessData ProcessData = nullptr;
		CacheThreadData ThreadData = nullptr;
		CacheExclusionsData ExclusionsData = nullptr;
	};

	//Cache data creation factories
	static inline CacheProcessData GetCacheProcessData()
	{
		return std::make_shared<SysmonXTypes::BaseCacheProcessData>();
	}

	static inline CacheThreadData GetCacheThreadData()
	{
		return std::make_shared<SysmonXTypes::BaseCacheThreadData>();
	}

	static inline CacheExclusionsData GetExclusionData()
	{
		return std::make_shared<SysmonXTypes::BaseCacheExclusionsData>();
	}
}
