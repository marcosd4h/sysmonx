...
namespace SysmonXHelpers
{
	...
	{
		EventPropertyID ret = EventPropertyID::EVENT_PROPERTY_ID_NA;

		if (!propertyName.empty())
		{
			if (propertyName == L"EventVersion")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventVersion;
			}
			else if (propertyName == L"EventID")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventVersion;
			}
			else if (propertyName == L"EventCollectorTechID")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventCollectorTechID;
			}
			else if (propertyName == L"EventCollectorVectorID")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventCollectorVectorID;
			}
			else if (propertyName == L"EventCreationTimestamp")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventCreationTimestamp;
			}
			else if (propertyName == L"EventETWProviderPID")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventETWProviderPID;
			}
			else if (propertyName == L"EventETWProviderName")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventETWProviderName;
			}
			else if (propertyName == L"CommandLine")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventCommandLine;
			}
			else if (propertyName == L"ScriptBlock")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_ScriptBlockText;
			}
			else if (propertyName == L"Image")
			{
				ret = EventPropertyID::EVENT_PROPERTY_ID_EventImage;
			}
	...
	//Due to return reference, we need to keep a static instance of a mMATCHING_TYPE_STRING global instance (again to avoid unncesary allocations)
	...
	const SysmonXTypes::MATCHING_TYPE_STRING &GetMatchingDataFromEvent(const SysmonXTypes::EventObject &eventData, const SysmonXTypes::EventPropertyID &eventID)
	{
		...
		if (eventData)
		{
			if (eventID == EventPropertyID::EVENT_PROPERTY_ID_EventETWProviderName)
			{
				return eventData->EventETWProviderName;
			}
			else if (eventID == EventPropertyID::EVENT_PROPERTY_ID_EventID)
			{
				return eventData->EventETWProviderName;
			}
			else if (eventID == EventPropertyID::EVENT_PROPERTY_ID_EventCommandLine)
			{
				return eventData->CommandLine;
			}
			else if (eventID == EventPropertyID::EVENT_PROPERTY_ID_ScriptBlockText)
			{
				return eventData->FreeText;
			}
			else if (eventID == EventPropertyID::EVENT_PROPERTY_ID_EventImage)
			{
				return eventData->Image;
			}
			else if (eventID == EventPropertyID::EVENT_PROPERTY_ID_EventETWProviderPID)
			{
