...
int SystesterHelpers::ToInteger(const std::wstring &str)
{
	return std::stoi(str);
}

void SystesterHelpers::WaitOnEnter()
{
	std::string dummy;
...
bool SystesterHelpers::IsValidFile(const std::wstring &fileName)
{
	bool ret = false;
...
bool SystesterHelpers::GetFullPathToFile(const std::wstring &fileName, std::wstring &fullPathFile)
{
	bool ret = true;
...
bool SystesterHelpers::IsNumber(const std::wstring& str)
{
	bool ret = false;
...
bool SystesterHelpers::GetFileToInjectSize(const std::wstring& file, DWORD &size)
{
	bool ret = false;
...
bool SystesterHelpers::ReadFileToInjectInBuffer(const std::wstring& file, const DWORD &fileSize, LPVOID lpBuffer, DWORD &bytesRead)
{
	bool ret = false;
...
bool SystesterHelpers::GetFileDetailsInBuffer(const std::wstring& file, LPVOID lpBuffer, DWORD &bytesRead)
{
	bool ret = false;
	...
	if ((!file.empty()) && 
		(SystesterHelpers::IsValidFile(file)) && 
		(SystesterHelpers::GetFileToInjectSize(file, workingFileSize)) &&
		(workingFileSize > 0))
	{
		...
		if ((lpWorkingBuffer != NULL) && 
			(SystesterHelpers::ReadFileToInjectInBuffer(file, workingFileSize, lpWorkingBuffer, workingBytesRead)) &&
...
bool SystesterHelpers::IsValidInjectionTarget(blackbone::pe::PEImage &sourceModule, blackbone::pe::PEImage &targetModule)
{
	bool ret = false;
...
bool SystesterHelpers::IsValidInjectionTarget(blackbone::pe::PEImage &codeToInject, blackbone::Process& targetToInject)
{
	bool ret = false;
...
bool SystesterHelpers::IsValidTargetPID(const std::wstring &targetPID)
{
	bool ret = false;

	if (SystesterHelpers::IsNumber(targetPID))
	{
		DWORD workingTargetPID = SystesterHelpers::ToInteger(targetPID);
		if ((workingTargetPID > PayloadCommon::DEFAULT_MINIMUM_USERSPACE_PID) && (workingTargetPID != GetCurrentProcessId()))
		{
			ret = true;
		}
	}
...
bool SystesterHelpers::GetExecutionContext(const std::wstring codeToInject, const std::wstring targetToInject, blackbone::pe::PEImage &sourceModule, blackbone::pe::PEImage &targetModule)
{
	bool ret = false;
...
bool SystesterHelpers::GetExecutionContext(const std::wstring codeToInject, const std::wstring targetToInject, blackbone::pe::PEImage &sourceModule, blackbone::Process &targetProc)
{
	bool ret = false;

	if ((SystesterHelpers::IsNumber(targetToInject)) &&
		(!codeToInject.empty()) &&
		(ERROR_SUCCESS == targetProc.Attach(SystesterHelpers::ToInteger(targetToInject))) &&
		(targetProc.valid()) &&
		(ERROR_SUCCESS == sourceModule.Load(codeToInject)) &&
		(!sourceModule.isExe()) &&
		(!sourceModule.path().empty()))
...
bool SystesterHelpers::StrContainsPatternInsensitive(const std::wstring& str, const std::wstring& pattern)
{
	bool ret = false;
...
PIMAGE_SECTION_HEADER SystesterHelpers::GetSectionOffset(const PVOID dwImageBase, const DWORD &offset)
{
...
bool SystesterHelpers::CreateSuspendedProcess(const std::wstring& targetProcess, 
...
bool SystesterHelpers::GetNTHeaders32(const PVOID execBuffer, IMAGE_NT_HEADERS32 &execNTHeader)
{
	bool ret = false;
...
bool SystesterHelpers::GetNTHeaders64(const PVOID execBuffer, IMAGE_NT_HEADERS64 &execNTHeader)
{
	bool ret = false;
...
bool SystesterHelpers::GetProcessCmdline(const HANDLE hProc, const PVOID procPEBAddr, std::wstring &cmdline)
{
	bool ret = false;
...
bool SystesterHelpers::GetRegStringValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, std::wstring &regContent)
{
	bool ret = false;
...
bool SystesterHelpers::SetRegStringValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, const std::wstring &regContent)
{
	bool ret = false;
...
bool SystesterHelpers::GetRegDWORDValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, DWORD &nValue, DWORD &nDefaultValue)
{
	bool ret = false;
...
bool SystesterHelpers::SetRegDWORDValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, const DWORD &nValue)
{
	bool ret = false;
...
bool SystesterHelpers::GetRegBoolValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, bool &nValue, bool &nDefaultValue)
{
	bool ret = false;
...
bool SystesterHelpers::SetRegBoolValue(const HKEY hRootKey, const std::wstring& regSubKey, const std::wstring& regValue, const bool &nValue)
{
	bool ret = false;
...
bool SystesterHelpers::ReverseString(const std::string &input, std::string &output)
{
	bool ret = false;
...
bool SystesterHelpers::ConvertStringToHexRepresentation(const std::string &input, std::string &output)
{
	bool ret = false;
...
bool SystesterHelpers::GetNearestRoundValue(const size_t &numToRound, const size_t &multiple, size_t &output)
{
	bool ret = false;
...
bool SystesterHelpers::PadStringWithValue(const std::string &input, const size_t num, const char paddingChar, std::string &output)
{
	bool ret = false;
...
bool SystesterHelpers::GetBaseFileName(const std::wstring& fullPath, std::wstring &fileName)
{
	bool ret = false;
